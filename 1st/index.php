...
function get1stSemesterResults() {
    ...
    try {
        ...
        $stmt1 = $pdo->prepare('SELECT roll_number, semester, GPA, grade FROM semester01_1st_2022 WHERE roll_number = ?');
        $stmt1->execute([$rollNumber]);
        $result1 = $stmt1->fetch(PDO::FETCH_ASSOC);

        $stmt2 = $pdo->prepare('SELECT roll_number, semester, GPA, grade FROM semester02_1st_2022 WHERE roll_number = ?');
